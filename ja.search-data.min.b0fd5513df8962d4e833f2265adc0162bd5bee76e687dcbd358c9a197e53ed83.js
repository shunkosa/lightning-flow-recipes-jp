"use strict";(function(){const t={encode:!1,tokenize:function(e){return e.replace(/[\x00-\x7F]/g,"").split("")}};t.doc={id:"id",field:["title","content"],store:["title","href","section"]};const e=FlexSearch.create("balance",t);window.bookSearchIndex=e,e.add({id:0,href:"/lightning-flow-recipes-jp/big-deal-chatter-alerts/",title:"商談成立時に Chatter 投稿とフォローアップ ToDo を作成",section:"Salesforce Flow Recipes",content:`
使い方 # フローを有効化し、金額が 1,000,000 円以上の商談を成立させると、そのレコードに Chatter 投稿され、ToDo が作成されます。
...
`}),e.add({id:1,href:"/lightning-flow-recipes-jp/disable-user-email-auto-bcc/",title:"ユーザ登録時にメールのBCC設定をオフにする",section:"Salesforce Flow Recipes",content:`
ユーザ登録時にメールのBCC設定をオフにするためのレコードトリガフロー(保存前フロー)です。
この項目はデフォルトでONになっていますが、このフローを有効化すると、登録時にOFFに設定されます。
ポイント # 保存前フローの基本的な内容については、『取引先の請求先住所を納入先住所に自動コピー』も合わせて確認してください。 同様の機能はプロセスビルダーや保存後フローでも実装できますが、実現したい処理が同じオブジェクトを更新するだけであれば、保存前フローの方が作りがシンプルになり、処理も高速です。 `}),e.add({id:2,href:"/lightning-flow-recipes-jp/mass-delete-leads/",title:"リストビューからリードを一括削除",section:"Salesforce Flow Recipes",content:`
...
`}),e.add({id:3,href:"/lightning-flow-recipes-jp/design-guideline/",title:"実装時に気をつけると良いこと",section:"Salesforce Flow Recipes",content:`Help に記載のベストプラクティス に可能な限り従ってください。
全体設計 # フローで実現したい処理が複雑な場合は、1 つのフローに何でも要素を詰め込むのではなく、サブフローに分割し、見通しのよいフローを作ることを心がけましょう。単に巨大な処理を分割するだけでなく、複数のフローで同じ処理を行いたい場合や、一部の処理で権限を昇格したい(例えば、ユーザ権限の画面フローで、システムオブジェクトにアクセスしたいなど)場合にもサブフローは活用できます。 変更される可能性がある値や、環境によって異なる値は、カスタム表示ラベルやカスタムメタデータに保存しましょう。要素内に直接値を設定するとメンテナンスが大変です。 1 オブジェクトにレコードトリガフロー、Apex トリガ、プロセス、ワークフロールールを混在させることを控えましょう。 フローやプロセス、ワークフロールールの機能比較、性能比較、ロードマップについては、『Architect’s Guide to Building Record-Triggered Automation』を参照してください。 既に組織のメインオブジェクトに複雑なプロセスビルダーや Apex トリガがある場合は、Winter \u0026lsquo;22 でレコードトリガフローでサブフローが対応されることを待ってから移行を検討すると良いでしょう。 命名規則 # 各要素の API 名には命名規則を設けることで保守性が高まります。このレシピでは一例として以下の命名規則を定めています。また、各要素の [説明] 欄には可能な限りその目的を入力しておきましょう。
...
フロー内で 1 度に実行できるレコードの操作には上限があるため、ループ内でのレコード操作を避け、コレクション変数を使用してレコードを操作してください。 詳細は Help 記事 を参照してください。 1 度に実行できるフロー要素の上限は 2000 のため、大きなコレクション変数をループする場合、この制限に抵触する可能性があります。ループ内のロジックを最適化できないか検討してください。 自動実行フローの場合は、[一時停止] 要素を使用することで処理を分けることができますが、このとき、フローの残りの部分は異なる処理(異なるフロートランザクション)として実行されます。 実行ユーザ # Summer \u0026lsquo;20 から開始要素でフローの実行ユーザを細かく制御できるようになりました。 自動起動フローは、呼び出し元の実行ユーザを引き継ぎます。プロセスからフローを起動する場合、プロセスの実行はシステムコンテキストとなるため、呼び出されるフローの実行ユーザもシステムコンテキストとなります。 権限制御 # フロー全体への権限については、[フローを実行] 権限やユーザの [フローユーザ] 項目を使用します。 単一のフローに対してアクセス制御を行たい場合は、各フローの [編集アクセス権] から、[デフォルト動作を上書きし、有効化されたプロファイルまたは権限セットにアクセスを制限します。] にチェックを入れ、アクセスできるプロファイルや権限セットを選択します。 フローの中で、特定の権限の場合に処理を実行したい場合は、決定要素にプロファイルやカスタム権限を条件に使用して、処理を分岐させると良いでしょう。 例: システム管理者だけが自分が作成したキャンペーンを一括削除できる画面 ユーザコンテキストでフローを実行する場合は、設定系のオブジェクトや管理者固有の項目など、一般ユーザが表示できないオブジェクトに対してどうしてもアクセスが必要な場合は、システム権限で実行されるサブフローにその処理を移しましょう。 `}),e.add({id:4,href:"/lightning-flow-recipes-jp/mass-campaign-delete-by-admin-screen/",title:"システム管理者だけが自分が作成したキャンペーンを一括削除できる画面",section:"Salesforce Flow Recipes",content:`プロファイルで表示する画面を分岐し、作成者が自身であるキャンペーンを一括削除するフローです。
...
`}),e.add({id:5,href:"/lightning-flow-recipes-jp/basic-login-flow/",title:"ログイン直後にお知らせメッセージを表示",section:"Salesforce Flow Recipes",content:`ログインフローの例です。
...
`}),e.add({id:6,href:"/lightning-flow-recipes-jp/task-with-file-create-screen/",title:"ToDoとファイルの登録画面",section:"Salesforce Flow Recipes",content:`新規 ToDo に続けてファイルをアップロードできる画面フローです。
...
`}),e.add({id:7,href:"/lightning-flow-recipes-jp/create-contact-update-case-from-web-or-email/",title:"Web-to-ケース/メール-to-ケースで取引先責任者を自動作成",section:"Salesforce Flow Recipes",content:`
Web-to-ケースまたはメール-to-ケースでケースを作成した場合、メールアドレスで取引先責任者が検索され、自動で紐付けされます。(参考: Salesforce Help - ケース登録時の関連付けについて) ただし、取引先責任者が見つからなかった場合は何も起こりません。このフローは、メールアドレスで取引先責任者を検索し、見つからなかった場合に取引先責任者を自動作成して紐付けします。
Summer \u0026lsquo;20 より保存後フローが利用できるようになったため、プロセスを使用せずにフローだけで実装することができます。
使い方 # Web-to-ケースまたはメール-to-ケースを組織で有効化します。 フローを有効化します。 取引先責任者のメールアドレスとして組織にレコードが存在しないメールアドレスを使用して、Web−to-ケースまたはメール-to-ケースでケースを作成します。 作成されたケースに、新規作成された取引先責任者が紐付いていることを確認します。 元のレコードを更新する # レコードトリガフローで元のレコードを更新する (今回の場合は元のケースを更新する) には、[フローをトリガした (オブジェクト名) レコードを使用] を選択します。これはプロセスビルダーで元のレコードを更新する際に、 [プロセスを開始した (オブジェクト名) レコードを使用] を選択するのと同じです。
`}),e.add({id:8,href:"/lightning-flow-recipes-jp/lead-update-screen/",title:"リードの更新画面",section:"Salesforce Flow Recipes",content:`リードを更新するシンプルな画面フローです。
...
`}),e.add({id:9,href:"/lightning-flow-recipes-jp/mass-email-to-contacts-screen/",title:"取引先から取引先責任者に一括メールを送信する画面",section:"Salesforce Flow Recipes",content:`取引先に紐づく取引先責任者に対して固定の一括メールを送信するサンプルのフローです。条件分岐や、コレクション変数、ループの使い方を学べます。
...
`}),e.add({id:10,href:"/lightning-flow-recipes-jp/account-opportunity-create-screen/",title:"取引先と商談を1度に作成する画面",section:"Salesforce Flow Recipes",content:`取引先と商談を1画面の入力内容から作成するフローです。
...
`}),e.add({id:11,href:"/lightning-flow-recipes-jp/account-create-screen/",title:"取引先の作成画面",section:"Salesforce Flow Recipes",content:`取引先を新規作成するシンプルな画面フローです。
...
`}),e.add({id:12,href:"/lightning-flow-recipes-jp/sync-account-billing-address-to-shipping-address/",title:"取引先の請求先住所を納入先住所に自動コピー",section:"Salesforce Flow Recipes",content:`
ポイント # 取引先の請求先住所を納入先住所に自動コピーするフローです。
...
`}),e.add({id:13,href:"/lightning-flow-recipes-jp/contact-search-and-update-screen/",title:"取引先責任者の検索・更新画面",section:"Salesforce Flow Recipes",content:`取引先責任者を名前で検索し、検索した取引先責任者のうちから、対象となるレコードを選択し、その取引先と説明欄を一括更新する画面フローです。
...
`}),e.add({id:14,href:"/lightning-flow-recipes-jp/opportunity-and-contact-role-create-screen/",title:"商談と取引先責任者ロールを1度に作成する画面",section:"Salesforce Flow Recipes",content:`商談と、その商談の取引先責任者のロール(最大2つまで)を1度に登録できる画面フローです。
...
`}),e.add({id:15,href:"/lightning-flow-recipes-jp/scheduled-task-email-reminder/",title:"毎朝9時に期日を過ぎた ToDo の所有者にメール通知するスケジュールフロー",section:"Salesforce Flow Recipes",content:`
ポイント # スケジュールトリガフロー # レポートのスケジューリングのように、決まった時間にフローを定期的に実行させるには、スケジュールトリガフローを使用します。新規フロー作成時に、 [スケジュールトリガフロー] を選択します。
